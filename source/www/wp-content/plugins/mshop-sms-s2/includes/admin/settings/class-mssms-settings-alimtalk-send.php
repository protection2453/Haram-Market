...
if ( ! class_exists( 'MSSMS_Settings_Alimtalk_Send' ) ) :
	...
	class MSSMS_Settings_Alimtalk_Send {
		...
		static function get_send_options() {
			...
			$options = array_diff_key( $options, array_flip( apply_filters( 'mssms_alimtalk_send_remove_order_status', array( 'pending' ) ) ) );
...
//											'sms'      => __( "문자알림 설정", 'mshop-sms-s2' ),
										)
									),
//									array(
//										"id"            => "sms_template_code",
//										"showIf"        => array( "resend_method" => "sms" ),
//										"title"         => __( "SMS/LMS 템플릿", 'mshop-sms-s2' ),
//										"className"     => "center aligned four wide column fluid",
//										"cellClassName" => "left aligned fluid",
//										"type"          => "Select",
//										"placeholder"   => "알림톡 템플릿을 선택하세요.",
//										"options"       => self::get_templates()
//									),
...
//									),
		...
		static function __get_user_setting() {
			ob_start();
			include( 'html/alimtalk-message-guide-for-user.php' );
			$guide = ob_get_clean();
			...
			return array(
				...
				'elements' => array(
					array(
						'type'     => 'Section',
						'title'    => __( '주문상태에 따른 알림톡 발송 설정', 'mshop-sms-s2' ),
						'elements' => array(
							array(
								"id"        => "mssms_alimtalk_user_options",
								"className" => "alimtalk_admin_option_table",
								'title'     => __( '고객 자동발송 설정', 'mshop-sms-s2' ),
								"type"      => "SortableTable",
								"repeater"  => true,
								"default"   => array(),
								"template"  => array(
					...
					array(
						'type'           => 'Section',
						'hideSaveButton' => true,
						'title'          => __( '메시지 작성 안내', 'mshop-sms-s2' ),
						'elements'       => array(
							array(
								"id"        => "mssms_alimtalk_user_options_desc",
								"className" => "fluid",
