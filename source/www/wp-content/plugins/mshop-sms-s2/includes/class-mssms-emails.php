...
if ( ! class_exists( 'MSSMS_Emails' ) ) {
	...
	class MSSMS_Emails {
		...
		static function maybe_send_point_shortage_notification( $response ) {
			...
			if ( MSSMS_Manager::use_point_shortage_notification() ) {
				$remain_point = isset( $response['remain_point'] ) ? intval( $response['remain_point'] ) : - 1;

				if ( $remain_point > 0 && $remain_point < 2000 && ! get_transient( 'mssms_point_shortage_notification' ) ) {
